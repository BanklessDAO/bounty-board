...
const useCachedForm = () => {
	const { data: cachedBounty } =
    useLocalStorage<typeof bountyFormFieldValues>('cachedEdit');
...
const NewBountyForm = (): JSX.Element => {
	...
	if (error) {
		console.warn(
			'Could not fetch the user data from discord - this will cause issues trying to create bounties'
		);
	}
	return (
		<form
			...
			<Button my="5" w="full" type="submit" disabled={!user}>
              Preview
			</Button>
			{!user && (
