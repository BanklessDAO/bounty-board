...
const useCustomerFromBounty = (
	bountyId: RouterKey
): { customer: CustomerProps | undefined; isLoading: boolean } => {
	...
	return {
		...
		isLoading:
      Boolean(bountyId) &&
      ((!bounty && !bountyError) || (!customer && !customerError)),
	};
};

const useCustomerFromKey = (
	customerKey: RouterKey
): { customer: CustomerProps | undefined; isLoading: boolean } => {
...
export const useCustomerFromBountyIdAndKey = (
	bountyId: RouterKey,
	customerKey: RouterKey
): { customer: CustomerProps | undefined; isLoading: boolean } => {
	/**
   ...
   */
	const { customer: customerFromKey, isLoading: customerFromKeyLoading } =
    useCustomerFromKey(customerKey);
	const { customer: customerFromBounty, isLoading: customerFromBountyLoading } =
    useCustomerFromBounty(bountyId);

	// customer name passed but matches no customer on file
	if (customerKey && !customerFromKey) {
		return {
			customer: undefined,
			isLoading: customerFromKeyLoading || customerFromBountyLoading,
		};
	}
	...
	if (
		customerFromKey &&
    customerFromBounty?.customerId !== customerFromKey?.customerId
	) {
		console.log(
	...
	return {
		customer: customerFromBounty,
		isLoading: customerFromKeyLoading || customerFromBountyLoading,
	};
};

export const setCustomerFromLocalStorage = (
...
export const useCustomer = (
  ...
  error: boolean;
	...
	const { customer: customerFromIdAndKey } = useCustomerFromBountyIdAndKey(
		id,
		customerKey
	);
